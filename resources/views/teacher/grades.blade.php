...
<div class="row">
 ...
 <button type="submit" id="generate" style="margin-left:0PX; margin-bottom: 20px;"class="btn btn-primary"> Generate Grade Sheet </button>
    {{ csrf_field() }}
    ...
    $.ajax({
        ...
        success: function (data){
            ...
            $.each(data.students, function(key, value){
                table.row.add([value.students.student_no, value.students.lastname + (',')+ value.students.firstname +  (' ') + value.students.middlename , value.mid_grade , value.final_grade, value.average_grade, value.grade_status,`<button class="btn btn-info" data-fullname="${value.students.lastname}" data-subject="${value.subjects.subject_description}"  data-section="${value.section.section_description}"  data-toggle="modal" class="btn btn-primary" data-midterm="${value.mid_grade}" data-finals="${value.final_grade}"  data-gradesid="${value.grade_id}"data-target="#grades" >Edit</button> | <a href="/studentgradepdf/${value.student_id}" class="btn btn-primary" ><span class="fa fa-print"></span> </a>`]);
